...
tools/                     สคริปต์ดึง/ตัดไอคอน (มี package.json แยก ไม่เกี่ยวกับ build ของเว็บ)
  ...
  build-icon-index.mjs     ทำดัชนีชื่อไฟล์ไอคอนไปเขียน iconFiles.ts
...
## ตัดไอคอนจากตารางสรุป (/summary)

ตารางสรุปของที่ต้องเก็บมีแต่ไอคอน ไม่ได้เขียนชื่อของไว้ ของหลายชิ้นจึงไม่มีในคลังไอคอนของเว็บ
แทนที่จะปล่อยให้ขึ้นเครื่องหมายคำถาม ให้ตัดไอคอนจากตารางมาใช้ตรง ๆ

```bash
node tools/crop-summary-icons.mjs desert-beach --sheet
```

ทำ contact sheet ติดเลขกำกับไว้ดูว่าไอคอนไหนคือลำดับที่เท่าไร เติม `SHOW_BOXES=1` ข้างหน้า
จะพิมพ์พิกัดของทุกก้อนออกมาด้วย ใช้เทียบว่าก้อนไหนอยู่คอลัมน์ไหนของตาราง

```bash
node tools/crop-summary-icons.mjs desert-beach
```

ตัดจริงลง `public/icons/summary/<zone>-NN.png` แล้วอ้างใน `src/data/summary.ts` ผ่านฟิลด์ `icon`
เลขลำดับเรียงบนลงล่าง ซ้ายไปขวา และคงที่ทุกครั้งที่รัน จึงอ้างเลขได้โดยไม่ต้องกลัวเลื่อน

**จุดสำคัญของวิธีนี้** คือ mask สร้างจากพิกเซล *ที่มีสี* ไม่ใช่หมึกทั้งหมด เพราะตารางมีข้อความ
อย่าง `x3 (CAT ONLY)` ปนอยู่เต็มไปหมด ตัวอักษรเป็นสีดำ/เทาไร้สีจึงหลุดออกไปเอง
(เคยลองใช้ ink mask แบบ `extract-icons.mjs` แล้วได้ไอคอนติดตัวเลขมาด้วยและจับได้แค่ 16 จาก 46 ชิ้น)

ผลข้างเคียง 2 อย่างที่ต้องรู้

- **ไอคอนสีอ่อน** (กระดาษขาว ขนนก หินเทา) สีจางเกินกว่าจะจับได้ ต้องระบุกรอบเองในตาราง `MANUAL`
  ที่หัวไฟล์ หาพิกัดด้วย `SHOW_BOXES=1` แล้วเทียบกับคอลัมน์ หรือลด `MIN_CHROMA=20` แต่จะเริ่มติดตัวอักษร
- **ตัวเลขสีแดง** อย่าง `+5` `x30` มีสีจัดพอจะถูกจับมาด้วย ไม่เป็นไรเพราะเราแค่ไม่อ้างเลขนั้น
...
เวลาเพิ่มไอคอนใหม่ ให้รัน `node tools/build-icon-index.mjs` เพื่ออัปเดตดัชนีชื่อไฟล์

```bash
node tools/build-icon-index.mjs
```

`findIcon` หาไอคอน 2 ชั้น ชั้นแรกคือตารางที่เขียนมือใน `src/data/icons.ts` ซึ่งใช้กติกา
"คีย์ที่ยาวที่สุดที่อยู่ในชื่อนั้นชนะ" (เช่น `Golden Mole Feather` ชนะ `Golden Mole`)
ชั้นที่สองคือดัชนีชื่อไฟล์จริงใน `src/data/iconFiles.ts` ที่สคริปต์ข้างบนสร้าง
ของที่ชื่อไฟล์ตรงกับชื่อของแบบ slug จึงเจอเองโดยไม่ต้องเพิ่มคีย์

ชั้นที่สองมีไว้กันพลาด — เคยมีไอคอน 18 ชิ้นที่ไฟล์อยู่ครบแต่ลืมเพิ่มคีย์ เลยไม่โผล่ในเว็บเลย
(Lotus Flower, Dream Note, Snail Shell ฯลฯ) ยังต้องเขียนคีย์เองอยู่เฉพาะกรณีที่ชื่อไม่ตรงกับไฟล์
เช่น `Legendary Recipe` ที่ใช้ไฟล์ `unknown-old-document` เพราะวิกิเรียกคนละชื่อ
